...
We adapted server side configurations to mitigate this, which in turn reduced the load on our EFS back to normals.
...
Main issue was that our EFS (elastic file system) mounted in the WMS and WMTS services was experience to high loads.
Among other ("invisible") stabilization measures we unmounted the file system for the WMS service.
This reduced the load significantly so that most mapviewer users would not experiences problems over the weekend.
