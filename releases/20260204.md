...
- Announcements:
    - The object ID values of the layer ch.vbs.schiessanzeigen have now changed for all objects due to geometrical changes in the source data. Existing permalinks to objects may not work properly anymore and should be replaced by the new object IDs (chsdi fields featureId and id).
    - The layer ch.bazl.luftfahrthindernis and also the corresponding download product asset luftfahrthindernis_4326.kmz will contain a significantly increased number of objects by the end of May 2026, because the high-voltage transmission lines will be represented as individual poles between the line segments as a refinement of this geodata product.
