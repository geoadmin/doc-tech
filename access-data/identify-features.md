...
| **limit (optional)**                 | The maximum number of results to retrieve per request (default limit=50, max limit=200). For layers that have multiple underlying tables registered in the system, the actual number of features returned may exceed the specified limit. The limit and offset is applied per table, meaning the total result set can contain up to limit × number_of_tables features.                                                         |
